    ...
    <div style={{ display: 'flex', gap: 2 }}>
      {[1,2,3,4,5].map(n => (
        <button key={n} type="button"
          ...
          style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: size, lineHeight: 1, padding: '1px', color: n <= (hover || value) ? '#B8912A' : 'var(--border-strong)', transition: 'color 0.1s' }}
        >★</button>
      ))}
    </div>
  )
...
function WineEntry({ wine, index, onChange, onRemove, cellarWines }) {
  ...
  return (
    <div style={{ background: 'var(--cream-dark)', borderRadius: 8, padding: '12px', marginBottom: 10, position: 'relative' }}>
      ...
      <div className="form-row" style={{ marginBottom: 8 }}>
        ...
        <div className="form-field"><label>{wine.source === 'Guest' ? 'Brought by' : 'Price ($)'}</label>
          {wine.source === 'Guest'
            ? <input value={wine.brought_by || ''} onChange={e => s('brought_by', e.target.value)} placeholder="Who brought it?" />
            : <input type="number" value={wine.price || ''} onChange={e => s('price', e.target.value)} placeholder="0.00" />
          }
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
        <div className="form-field" style={{ flex: 'none' }}><label>Rating</label><StarRating value={wine.rating || 0} onChange={v => s('rating', v)} size={20} /></div>
      </div>
      <div className="form-field"><label>Tasting note</label>
      ...
      {(wine.rating >= 4) && (
        <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
          <button type="button"
            onClick={() => s('drunk_hero', !wine.drunk_hero)}
            ...
            🏆 {wine.drunk_hero ? 'Drunk Hero ✓' : 'Add to Drunk Heroes'}
          </button>
          <button type="button"
            onClick={() => s('reorder', !wine.reorder)}
            ...
            🔁 {wine.reorder ? 'Want List ✓' : 'Add to Want List'}
          </button>
        </div>
      )}
    </div>
...
export default function DiningForm({ initial = {}, scannedWines = [], onSave, onCancel }) {
  ...
  const [wines, setWines] = useState(scannedWines.length ? scannedWines.map(w => ({ ...w, source: 'Restaurant list', rating: 0, drunk_hero: false, reorder: false })) : [])

  const s = (k, v) => setF(p => ({ ...p, [k]: v }))

  const addWine = () => setWines(prev => [...prev, { producer: '', wine_name: '', vintage: '', type: 'Red', source: 'Restaurant list', rating: 0, drunk_hero: false, reorder: false }])
  const removeWine = (i) => setWines(prev => prev.filter((_, idx) => idx !== i))
  const updateWine = (i, wine) => setWines(prev => prev.map((w, idx) => idx === i ? wine : w))
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div className="form-row">
        <div className="form-field">
          <label>{f.type === 'Home' ? 'Location / area' : f.type === 'Event' ? 'Event / venue name' : 'Restaurant name'}</label>
          <input value={f.venue} onChange={e => s('venue', e.target.value)} placeholder={f.type === 'Home' ? 'e.g. Home, Claremont' : f.type === 'Event' ? 'e.g. Penfolds Magill Estate Tasting' : 'e.g. Loulou'} />
        </div>
        {f.type === 'Restaurant' && (
      ...
      {wines.map((w, i) => (
        <WineEntry key={i} wine={w} index={i} onChange={updateWine} onRemove={removeWine} />
      ))}

      <button type="button" onClick={addWine} className="btn btn-secondary btn-sm" style={{ alignSelf: 'flex-start' }}>
        + Add wine
      </button>

      {f.type !== 'Event' && (
        <>
          <div style={{ fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--ink-light)', paddingBottom: 4, borderBottom: '1px solid var(--border)', marginTop: 4 }}>Food</div>
          <div className="form-row full">
            <div className="form-field"><label>Food notes</label>
              <textarea value={f.food_notes} onChange={e => s('food_notes', e.target.value)} placeholder="What did you eat? Standout dishes…" rows={2} />
            </div>
          </div>
      ...
      <div className="modal-footer">
        ...
        <button className="btn btn-primary" onClick={() => onSave(f, wines)}
          disabled={!f.venue && !f.date}>
          Save entry
        </button>
      </div>
    </div>
