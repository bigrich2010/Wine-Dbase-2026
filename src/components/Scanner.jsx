...
export default function Scanner({ onScanned, onCancel }) {
  ...
  const handleFile = e => {
    ...
    r.onload = ev => analyse(ev.target.result)
...
Use broad region (e.g. "Margaret River", "Burgundy") and specific appellation (e.g. "Wilyabrup Valley", "Gevrey-Chambertin").`
  ...
  if (phase === 'camera') return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button className="btn btn-primary" style={{ flex: 1 }} onClick={capture}>📷 Capture label</button>
  ...
  if (phase === 'error') return (
    <div style={{ textAlign: 'center', padding: '1rem 0' }}>
      <p style={{ color: '#A32D2D', marginBottom: 16, fontSize: 13 }}>Couldn't read the label — try again or add manually.</p>
